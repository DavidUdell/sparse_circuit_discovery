...
def mock_slices():
    ...
    slices: list[slice] = [
        slice(0, 1), slice(1, 2), slice(0, 2, 1), slice(0, None, 2),
        slice(None, None), slice(None, None), slice(0, None), slice(None, 1),
        slice(2, None), slice(None, None, 5), slice(None, 3), slice(-1, None),
        slice(0, 9), slice(2, None, 2), slice(0, 5, -1), slice(None, None, -1),
    ]
    # Pythia 70M has 6 layers.
    ranges: list[range] = [
        range(0, 1), range(1, 2), range(0, 2, 1), range(0, 6, 2),
        range(0, 6), range(0, 6), range(0, 6), range(0, 1),
        range(2, 6), range(0, 6, 5), range(0, 3), range(5, 6),
        range(0, 6), range(2, 6, 2), range(0, 5, -1), range(0, 6, -1),
    ]
...
def test_slice_to_seq(mock_slices):  # pylint: disable=redefined-outer-name
    ...
    for parsed_slice, model_range in zip(slices, ranges):
        assert slice_to_seq(model, parsed_slice) == model_range, (
            dedent(
                f"""
